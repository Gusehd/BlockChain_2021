#19101185 강동현 - 3015 Modes of Operation - AES 128
...
if __name__ == "__main__":
    ...
    if mode == "CBC":
        ...
        cp = ""
        for i in range(0, n):
            app = plaintext[32 * i:32 * i + 32]
            plain_stack.append(app)
        first = xor(iv, plain_stack[0])
        cp1 = main(first,key)
        cp = cp + cp1
        for i in range(1,n):
            inputs = xor(cp1,plain_stack[i])
            cp1 = main(inputs,key)
            cp = cp + cp1
        print(cp)
    else:
        ...
        start = int("0x"+ctr,16)
        for i in range(0,n):
            hexcheck = hex(start + i).replace("0x","")
            if len(hexcheck) < 32:
                hexcheck = "0"*(32-len(hexcheck)) + hexcheck
            ctr_stack.append(hexcheck)

        for i in range(0,n):
            aes_output = main(ctr_stack[i],key)
            cps = xor(aes_output,plain_stack[i])
            cp = cp + cps
        print(cp)
